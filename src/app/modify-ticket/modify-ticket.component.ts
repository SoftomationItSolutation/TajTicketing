...
export class ModifyTicketComponent implements OnInit {
  ...
  SloatData(){
    this.objDbServ.GetSlotByEntryData({VisitDate:this.VisitDate}).subscribe(
      (response: Response)=>{
  ...
  GetTicketById(){
    this.objDbServ.CommonGetData({Id:this.SelectedVisiterId,Flag:'TicketById'}).subscribe(
      (response: Response)=>{
        ...
        if(data.length>0)
        {
          this.VisitorName= data[0].VisiterName,
          this.GenderId= data[0].VisiterGender,
          this.VisitorAge= data[0].VisiterAge,
          this.VisitDate= data[0].VisitDate,
          this.PassportNo= data[0].VisiterPassportNo,
          this.Address= data[0].VisiterAddress1,
  ...
  onSubmit() {
    ...
    this.objDbServ.ModifyTicket(obj).subscribe(
      (response: Response)=>{
